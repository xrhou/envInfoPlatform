...
<head>
    ...
    $(function(){
    	...
    	grid=$("#maingrid4").ligerGrid({
        	...
        	dataAction: 'local',
			url: "<%=request.getContextPath()%>/jsonfindAllEntPollutionInfoByEntUserId?entUserId="+entUserId+"&gasOrWater="+gasOrWater,
			pageSize: 17,
	    ...
	    function showRow(){
    		...
    		if (null==rowData) {
    			...
    			$.ligerDialog.open({
    				title:'查看单位水污染物排放信息',
    				width: 790, height: 265,
    				url: url+'watchEntPollutionInfoByPollutionInfoId?pollutionInfoId='+encodeURIComponent(rowData.pollutionInfoId),
        ...
        function addRow() {	
    		...
    		$.ligerDialog.open({
    			title:'添加单位水污染物排放信息',
    			width: 790, height: 265,
    			url:url+'admin_addWaterPollution.jsp',
    			isResize: true, top: 50 
    		});
        }
    	...
    	function updateRow(){
    		...
    		if (null==rowData) {
    			...
    			$.ligerDialog.open({
    				title:'修改单位水污染物排放信息',
    				width: 790, height: 260,
    				url: url+'getEntPollutionInfoByPollutionInfoId?pollutionInfoId='+encodeURIComponent(rowData.pollutionInfoId),
    				isResize: true, top: 50
    			});
    		}
    	}
    			...
    			$.ligerDialog.confirm('亲,是否确定删除？', function (type){
    		    	if(type==true){
    		    		$.ajax({
    		    	      	...
    		    	      	url: url+"deleteEntPollutionInfoByPollutionInfoId", 
    		    	      	isAjaxSubmit: true,
...
<body style="padding:2px; overflow:hidden;">
	...
	<input type="hidden" name="gasOrWater" id="gasOrWater" value="W"/>
