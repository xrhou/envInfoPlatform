		...
		function formSubmit(){
			...
			if(pMouth.length<=0){
				...
				$.ajax({
					type: "POST",
					dataType:"text" ,
					data:"entUserId="+entUserId+"&accountTotalEmission="+accountTotalEmission+"&mgL="+mgL+"&monitorDate="+monitorDate+"&monitorMothod="+monitorMothod+
						"&pLocation="+pLocation+"&pMethod="+pMethod+"&pMouth="+pMouth+"&pName="+pName+"&standartMGL="+standartMGL+"&totalEmission="+totalEmission+"&over="+over+"&gasOrWater="+gasOrWater,
					url:"<%=request.getContextPath()%>/addEntPollutionInfoByPollutionInfoId", 
					isAjaxSubmit: true,
					...
					success: function(data){
						if(data=="success"){
							...
							parent.$.ligerDialog.warn("添加水污染物排放信息失败!");
						}
					},
					error: function() {
						parent.$.ligerDialog.warn("服务器出错!请联系管理员.");
					},
				});
				return false;
			}
...
<body style="padding:10px">
    ...
    <input name="gasOrWater" type="hidden" id="gasOrWater" value="W"/>
     <table border="0" cellpadding="0" cellspacing="0" class="l-table-edit" >
            ...
            <tr>
                ...
                <td align="left" class="l-table-edit-td">
                	<input name="pMouth" type="text" id="pMouth" />
                </td>
                <td align="right" class="l-table-edit-td">排放口位置:</td>
                <td align="left" class="l-table-edit-td">
                    <input id="pLocation" type="text" name="pLocation" />[经纬度]
                </td>
            </tr>
            <tr>
                ...
                <td align="left" class="l-table-edit-td" width="320px">
	                <select name="pMethod" id="pMethod">
						<option value="">选择排放方式</option>
						<option value="纳管">纳管</option>
						<option value="排外环境">排外环境</option>
					</select>
                </td>
            ...
            <tr>
                ...
                <td align="left" class="l-table-edit-td">
                    <input name="standartMGL" type="text" id="standartMGL"/>[mg/L]
                </td>
                <td align="right" class="l-table-edit-td">监测时间:</td>
                <td align="left" class="l-table-edit-td">
                    <input name="monitorDate" type="text" id="monitorDate" class="Wdate" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})"/>
                </td>
            </tr>
            <tr>
            	<td align="right" class="l-table-edit-td">排放总量:</td>
                <td align="left" class="l-table-edit-td">
                    <input name="totalEmission" type="text" id="totalEmission"/>[Kg]
                </td>
                <td align="right" class="l-table-edit-td">核定的排放总量:</td>
                <td align="left" class="l-table-edit-td"> 
                	<input name="accountTotalEmission" type="text" id="accountTotalEmission"/>[Kg]
                </td>
            </tr>
            <tr>
